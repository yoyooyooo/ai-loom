...
- 组件尽量“无状态/无副作用”：复杂副作用与状态迁移下沉到 Store Action 或 Service Hook；组件只消费 Store 状态与动作；Store 如果负责了多个领域的逻辑，拆分为多个 Slice 后聚合到一起。
- 条件渲染统一使用 ts-pattern：组件内按“类型/状态”分支渲染，采用 `ts-pattern` 的 `match().with().otherwise()`，避免深层三元/if-else/switch。复杂片段抽成纯函数后在 `match` 中组合调用。
- Query × RxJS 融合：
  - React Query → 查询类接口与快照（config/列表/history 等）。
  - RxJS → 流式/轮询/WS；产出的最新快照同时写入 QueryClient 缓存（`setQueryData`），查询侧也能直接消费。
  - 轮询需有双保险：最大时长 + 无增量次数阈值。

- 开发与约定
  - 默认“读取优先 WS”（写入 REST）；必要时通过 `VITE_WS_NO_FALLBACK=1`/`VITE_WS_WRITE=1` 验证纯 WS；调试 `VITE_WS_DEBUG=1`、`VITE_WS_DEBUG_ROUTE=1`。
  - 前端类型检查：`just web-typecheck`；前端单测：`just web-test`；前端本地开发：`just web-dev`；后端热重载：`just server-dev`；一键联动：`just dev-all`。
