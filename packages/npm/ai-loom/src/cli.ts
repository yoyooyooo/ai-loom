...
let familySync: null | (() => string) = null
let MUSL = 'musl'
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const dl = require('detect-libc') as { familySync?: () => string; MUSL?: string }
  familySync = typeof dl.familySync === 'function' ? dl.familySync : null
  MUSL = dl.MUSL || 'musl'
} catch (_) {}

function pickPackage(): string {
...
async function main() {
  ...
  // 仅透传“未知选项”给后端；去除我们自己的 CLI 选项（例如 --watch / -w）
  const passthrough = (program.parseOptions(process.argv).unknown || []).filter((a: string) => a !== '--watch' && a !== '-w')
...
main().catch((e: any) => {
  console.error('[ai-loom]', e?.message || e)
  process.exit(1)
})
