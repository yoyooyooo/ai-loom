...
describe('subscribeChatEvents', () => {
  ...
  it('processes codex runtime events and keeps single user message', () => {
    ...
    // 开始推理 + 回答
    __emit('codex/event/agent_reasoning_section_break', {
      conversationId: 'conv-1',
      msg: { type: 'agent_reasoning_section_break' }
    })
    __emit('codex/event/agent_reasoning_delta', {
      conversationId: 'conv-1',
      msg: { type: 'agent_reasoning_delta', delta: '**Preparing**' }
    })
    __emit('codex/event/agent_message_delta', {
      conversationId: 'conv-1',
      msg: { type: 'agent_message_delta', delta: '答复中' }
    })
    __emit('codex/event/agent_message', {
      conversationId: 'conv-1',
      msg: { type: 'agent_message', message: '完成' }
    })
    __emit('codex/event/task_complete', {
      conversationId: 'conv-1',
