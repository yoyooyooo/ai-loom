...
describe('WS → turns 行为覆盖（缺失边界与异常兜底）', () => {
  ...
  it('缺失 chat.turn.started：message.delta 隐式开启；completed 立即结束该轮（Working=Finished）', () => {
    ...
    expect(st.activeTurnId).toBeUndefined()
  })
  ...
  it('工具并发：exec + mcp 各自聚合并完成', () => {
    ...
    __emit('chat.tool.exec.begin', { conversationId: 'conv-t', callId: 'c1', command: ['bash','-lc','echo 1'] })
    __emit('chat.tool.mcp.begin', { conversationId: 'conv-t', callId: 'm1', server: 'sv', tool: 'tl', arguments: { x: 1 } })
    __emit('chat.tool.exec.output', { conversationId: 'conv-t', callId: 'c1', text: '1' })
    __emit('chat.tool.exec.end', { conversationId: 'conv-t', callId: 'c1', exitCode: 0, stdout: '1' })
  ...
  it('异常：patch 无 callId begin + 无 callId end → 结束不了（保持 Working）', () => {
    ...
    __emit('chat.tool.patch.begin', { conversationId: 'conv-p', files: 2, firstPath: 'a.txt', autoApproved: true })
  ...
  it('reasoning.section_break 追加分隔符', () => {
    stop = subscribeChatEvents()
    chatTurnActions.setConversationId('conv-r')
    __emit('chat.turn.started', { conversationId: 'conv-r' })
    __emit('codex/event/agent_reasoning_section_break', { conversationId: 'conv-r', msg: { type: 'agent_reasoning_section_break' } })
    const t = useChatTurnStore.getState().turns[0]
    expect((t.reasoning?.content || '')).toContain('---')
  })
})
