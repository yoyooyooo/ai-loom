...
describe('UI：工具步骤聚合与渲染（exec/mcp/patch）', () => {
  ...
  it('exec/mcp/patch 的封面标题与标签、diff 内容正确渲染', async () => {
    ...
    // exec
    __emit('chat.tool.exec.begin', { conversationId: 'conv-ui', callId: 'c1', command: ['bash', '-lc', 'echo hi'], cwd: '/tmp' })
    __emit('chat.tool.exec.output', { conversationId: 'conv-ui', callId: 'c1', text: 'hi\n', stream: 'stdout' })
    __emit('chat.tool.exec.end', { conversationId: 'conv-ui', callId: 'c1', exitCode: 0, stdout: 'hi\n' })

    // mcp
    __emit('chat.tool.mcp.begin', { conversationId: 'conv-ui', callId: 'm1', server: 's', tool: 't', arguments: { q: 'x' } })
    __emit('chat.tool.mcp.end', { conversationId: 'conv-ui', callId: 'm1', server: 's', tool: 't', result: { ok: true } })
