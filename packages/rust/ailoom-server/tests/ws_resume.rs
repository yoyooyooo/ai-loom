...
async fn ws_events_resume_after_write() {
  ...
  let req = serde_json::json!({"jsonrpc":"2.0","id":1,"method":"events.resume","params":{"after":0}});
  ...
  let events = resp.get("result").and_then(|r| r.get("events")).and_then(|e| e.as_array()).cloned().unwrap_or_default();
  assert!(events.iter().any(|ev| ev.get("method").and_then(|m| m.as_str()) == Some("file.changed")));
}
