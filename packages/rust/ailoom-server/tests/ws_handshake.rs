...
async fn ws_welcome_handshake() {
  ...
  let txt = match msg { Message::Text(s) => s, _ => panic!("unexpected first frame") };
  let v: serde_json::Value = serde_json::from_str(&txt).unwrap();
  assert_eq!(v.get("method").and_then(|m| m.as_str()), Some("session.welcome"));
  let limits = v.get("params").and_then(|p| p.get("limits")).cloned().unwrap();
