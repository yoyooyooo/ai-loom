...
async fn rpc_tree_get_and_broadcast_on_file_put() {
    ...
    assert!(
        status.is_success(),
        "REST tree should work, got {} body={} ",
        status,
        body
    );
    ...
    // call tree.get
    let req =
        serde_json::json!({"jsonrpc":"2.0","id":1,"method":"tree.get","params":{"dir":"src"}});
    ...
    assert!(
        ...
        "tree.get should return result: {resp_txt}"
    );
    ...
    let put_body =
        serde_json::json!({"path":"src/a.txt","content":"hello\nworld!!!\n","baseDigest":null});
    ...
    for _ in 0..20 {
        // up to ~2s
        if let Some(Ok(Message::Text(t))) = read.next().await {
            if t.contains("\"method\":\"file.changed\"") && t.contains("src/a.txt") {
                got_changed = true;
                break;
            }
        }
