...
async fn ws_file_save_ok_and_conflict() {
  ...
  let resp_ff_txt = loop { if let Some(Ok(Message::Text(t))) = read.next().await { if t.contains("\"id\":1") { break t; } } else { panic!("no response"); } };
  let resp_ff: serde_json::Value = serde_json::from_str(&resp_ff_txt).unwrap();
  let digest = resp_ff.get("result").and_then(|r| r.get("digest")).and_then(|d| d.as_str()).unwrap().to_string();

  // save ok
  let req_save = serde_json::json!({"jsonrpc":"2.0","id":2,"method":"file.save","params":{"path":"a.txt","content":"hello world\n","baseDigest":digest}});
  write.send(Message::Text(req_save.to_string())).await.unwrap();
  let resp_save_txt = loop { if let Some(Ok(Message::Text(t))) = read.next().await { if t.contains("\"id\":2") { break t; } } else { panic!("no response"); } };
  let resp_save: serde_json::Value = serde_json::from_str(&resp_save_txt).unwrap();
  assert!(resp_save.get("result").and_then(|r| r.get("ok")).and_then(|v| v.as_bool()).unwrap_or(false));
  ...
  write.send(Message::Text(req_conf.to_string())).await.unwrap();
  let resp_conf_txt = loop { if let Some(Ok(Message::Text(t))) = read.next().await { if t.contains("\"id\":3") { break t; } } else { panic!("no response"); } };
  let resp_conf: serde_json::Value = serde_json::from_str(&resp_conf_txt).unwrap();
  let err_code = resp_conf.get("error").and_then(|e| e.get("code")).and_then(|c| c.as_str()).unwrap_or("");
