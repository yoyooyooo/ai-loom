use crate::{paths::to_workspace_relative, state::AppState};
...
pub async fn verify_annotations_for_file(
    ...
    match ailoom_fs::read_file_full(&state.fs, root_rel_path) {
        ...
        Err(e) => {
                ...
                || msg == "NOT_FILE"
            {
                ...
                tracing::warn!(
                    "verify: read_file_full failed for {}: {}",
                    root_rel_path,
                    msg
                );
            }
    ...
    fn slice_by_char_cols(line: &str, start_col_1b: usize, end_col_1b: usize) -> String {
        ...
        for (ci, (bi, _ch)) in line.char_indices().enumerate() {
            if start_byte.is_none() && ci == start_char {
                start_byte = Some(bi);
            }
            if ci == end_char {
                end_byte = Some(bi);
    ...
    for mut ann in target.drain(..) {
        ...
        if ann.selected_text.trim().is_empty() {
            if remove_broken {
                if let Err(e) = state.store.delete_annotation(&ann.id).await {
                    tracing::warn!(
                        "verify: delete(empty-selected) failed for {}: {}",
                        ann.id,
                        e
                    );
                } else {
                    deleted += 1;
                    deleted_ids.push(ann.id.clone());
                }
            } else {
                skipped += 1;
                skipped_ids.push(ann.id.clone());
            }
            continue;
        }
        ...
        let win_start = if ann.start_line > 1 {
            (ann.start_line as usize).saturating_sub(1)
        } else {
            1
        };
        ...
        match ailoom_fs::read_file_chunk(&state.fs, root_rel_path, win_start, need) {
            Ok(ch) => {
                ...
                let s_rel = (ann.start_line as usize)
                    .saturating_sub(ch.start_line)
                    .saturating_add(1)
                    .max(1);
                let e_rel = (ann.end_line as usize)
                    .saturating_sub(ch.start_line)
                    .saturating_add(1)
                    .max(1)
                    .min(lines.len());
        ...
        if let Ok(ch) =
            ailoom_fs::read_file_chunk(&state.fs, root_rel_path, win_start2.max(1), max_lines)
        {
            ...
            if !occs.is_empty() {
                ...
                for o in occs.into_iter().skip(1) {
                    let d = o.0.abs_diff(anchor);
                    if d < best_dist {
                        best = o;
                        best_dist = d;
                    }
        ...
        if new_pos.is_none() {
            if ann.selected_text.contains('\n') {
                if let Ok(ch2) = ailoom_fs::read_file_chunk(
                    &state.fs,
                    root_rel_path,
                    win_start2.max(1),
                    max_lines,
                    ...
                    if !head.is_empty() && !tail.is_empty() {
                        ...
                        if let Some((s_abs, s_col1, _)) = best_s {
                            ...
                            for (j, ln) in lines_vec
                                .iter()
                                .enumerate()
                                .skip(s_abs.saturating_sub(ch2.start_line))
                            {
                            ...
                            if let Some((e_abs, _e_scol, e_col1)) = best_e {
                                if e_abs >= s_abs {
                                    new_pos = Some((s_abs, e_abs, s_col1, e_col1));
                                }
                            }
                        }
                    }
        ...
        if let Some((s_line, e_line, s_col, e_col)) = new_pos {
            ...
            if ann.start_column.is_some() {
                ann.start_column = Some(s_col as i64);
            }
            if ann.end_column.is_some() {
                ann.end_column = Some(e_col as i64);
            }
            ann.updated_at = time::OffsetDateTime::now_utc()
                .format(&time::format_description::well_known::Rfc3339)
                .unwrap_or_else(|_| ann.updated_at);
            if let Some(ref d) = full_digest {
                ann.file_digest = Some(d.clone());
            }
            if let Err(e) = state.store.update_annotation(&ann).await {
                tracing::warn!("verify: update failed for {}: {}", ann.id, e);
            } else {
                updated += 1;
                updated_ids.push(ann.id.clone());
            }
...
mod tests {
    ...
    fn write_file(p: &PathBuf, content: &str) {
    ...
    async fn make_state(tmpdir: &PathBuf) -> AppState {
        ...
        let store = Store::connect_path(&db, &workspace_root.to_string_lossy())
            .await
            .unwrap();
        AppState {
            fs: fs_cfg,
    ...
    async fn verify_deletes_when_not_found() {
        ...
        let r =
            verify_annotations_for_file(&state, file_rel, Some(40), Some(5 * 1024 * 1024), true)
                .await
                .unwrap();
