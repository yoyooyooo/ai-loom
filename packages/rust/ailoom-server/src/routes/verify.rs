use crate::{
    services::verification::{verify_annotations_for_file, VerifyResultOut},
    state::AppState,
    web::error,
};
...
pub async fn verify_annotations_endpoint(
    ...
    let file = match body.file_path.as_deref() {
        ...
        None => {
            return (
                ...
                Json(error("INVALID", "filePath is required")),
            )
                .into_response();
        }
    };
    ...
    match verify_annotations_for_file(&state, &file, Some(window), Some(full_limit), remove_broken)
        .await
    {
