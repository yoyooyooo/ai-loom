...
impl Hub {
  pub fn new(buffer: usize) -> Self {
    ...
    let dedup_ms = std::env::var("AILOOM_WS_DEDUP_MS").ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or(200);
    Self {
      tx,
  ...
  pub fn broadcast(&self, mut method: String, mut params: Value) {
    ...
    if let Some(obj) = params.as_object_mut() {
      ...
      obj.entry("eventId").or_insert_with(|| Value::String(id.to_string()));
    }
    // QoS: ring buffer prioritization - avoid evicting high-priority events
    {
      ...
      if ring.len() >= self.ring_cap {
        if method == "tree.changed" {
          ...
          self.stats.dropped_ring_lowpri.fetch_add(1, Ordering::Relaxed);
        } else {
          // try to evict a low-priority tree.changed first
          let pos = ring.iter().position(|e| e.method == "tree.changed");
          if let Some(i) = pos { ring.remove(i); } else { ring.pop_front(); }
          ring.push_back(EventRecord { id, method: method.clone(), params: params.clone() });
        }
      } else {
        ring.push_back(EventRecord { id, method: method.clone(), params: params.clone() });
      }
    ...
    let sent = self.tx.send(Event { method: std::mem::take(&mut method), params });
    match sent {
      Ok(n) => {
        self.stats.broadcast_total.fetch_add(1, Ordering::Relaxed);
        if n == 0 { self.stats.no_receiver.fetch_add(1, Ordering::Relaxed); }
  ...
  pub fn inc_file_changed(&self) {
    self.stats.file_changed_total.fetch_add(1, Ordering::Relaxed);
  }

  pub fn report_tree_changed(&self, impacted_count: usize, moved_count: usize, truncated: bool) {
    self.stats.tree_changed_batches.fetch_add(1, Ordering::Relaxed);
    self.stats.tree_impacted_paths_total.fetch_add(impacted_count as u64, Ordering::Relaxed);
    self.stats.tree_moved_total.fetch_add(moved_count as u64, Ordering::Relaxed);
    if truncated { self.stats.tree_truncated_batches.fetch_add(1, Ordering::Relaxed); }
  }
