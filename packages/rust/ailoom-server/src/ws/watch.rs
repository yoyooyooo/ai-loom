...
fn read_cfg() -> Cfg {
  ...
  let batch_ms = std::env::var("AILOOM_FSWATCH_BATCH_MS").ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or(300);
  let max_window_ms = std::env::var("AILOOM_FSWATCH_MAX_WINDOW_MS").ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or(1000);
  let max_impacted_paths = std::env::var("AILOOM_FSWATCH_MAX_IMPACTED").ok().and_then(|s| s.parse::<usize>().ok()).unwrap_or(200);
  let ignore_vcs = std::env::var("AILOOM_FSWATCH_IGNORE_VCS").unwrap_or_else(|_| "1".into()) == "1";
  let ignore_ailoom = std::env::var("AILOOM_FSWATCH_IGNORE_AILOOM").unwrap_or_else(|_| "1".into()) == "1";
  Cfg { enabled, batch_ms, max_window_ms, max_impacted_paths, ignore_vcs, ignore_ailoom }
}
...
pub fn spawn_watcher(state: AppState) -> Option<tokio::task::JoinHandle<()>> {
  ...
  let hub = match state.ws_hub.clone() { Some(h) => h, None => return None };
  tracing::info!(target: "fswatch", "starting watcher on {}", root.display());

  let (tx, rx) = std::sync::mpsc::channel::<notify::Result<Event>>();
  let mut watcher = match RecommendedWatcher::new(tx, notify::Config::default()) { Ok(w) => w, Err(e) => { tracing::warn!(target: "fswatch", "failed to init watcher: {}", e); return None; } };
  ...
  let handle = tokio::spawn(async move {
    ...
    loop {
      ...
      match got {
        Ok(Ok(ev)) => {
          pending.push(ev);
          if win_start.is_none() { win_start = Some(Instant::now()); }
        }
      ...
      // flush condition
      if !pending.is_empty() {
        let elapsed = win_start.map(|s| s.elapsed().as_millis() as u64).unwrap_or(0);
...
fn process_batch(hub: &crate::ws::hub::Hub, root: &PathBuf, cfg: &Cfg, filter: Option<&IgnoreFilter>, batch: Vec<Event>) {
  ...
  for ev in batch.into_iter() {
    // collect paths
    let paths: Vec<String> = ev.paths.iter().filter_map(|p| to_root_rel(root, p)).filter(|p| !is_ignored(filter, p)).collect();
    if paths.is_empty() { continue; }
    match ev.kind {
      EventKind::Create(_) => {
        for p in paths { created.insert(p); }
      }
      EventKind::Modify(modk) => {
        ...
        match modk {
            ...
            match nm {
              ...
              RenameMode::Both => {
                if ev.paths.len() >= 2 {
                  ...
                  if !from.is_empty() && !to.is_empty() { moved.push((from, to)); }
                }
              }
              _ => {}
            }
          ...
          _ => {
            for p in paths { modified.insert(p); }
          }
        }
      }
      EventKind::Remove(_) => {
        for p in paths { deleted.insert(p); }
      }
      _ => {}
    }
  }
  ...
  for p in deleted.iter() { created.remove(p); modified.remove(p); }
  for p in created.iter() { modified.remove(p); }

  // impacted paths for tree summary
  let mut impacted: HashSet<String> = HashSet::new();
  for p in created.iter() { impacted.insert(p.clone()); }
  for p in modified.iter() { impacted.insert(p.clone()); }
  for p in deleted.iter() { impacted.insert(p.clone()); }
  for (from, to) in moved.iter() { impacted.insert(from.clone()); impacted.insert(to.clone()); }

  let mut impacted_list: Vec<String> = impacted.into_iter().collect();
  impacted_list.sort();
  let truncated = impacted_list.len() > cfg.max_impacted_paths;
  if truncated { impacted_list.truncate(cfg.max_impacted_paths); }
  ...
  for p in created.iter() {
    hub.broadcast("file.changed".into(), serde_json::json!({"path": p, "kind":"created"}));
    hub.inc_file_changed();
  }
  for p in modified.iter() {
    hub.broadcast("file.changed".into(), serde_json::json!({"path": p, "kind":"modified"}));
    hub.inc_file_changed();
  }
  for p in deleted.iter() {
    hub.broadcast("file.changed".into(), serde_json::json!({"path": p, "kind":"deleted"}));
    hub.inc_file_changed();
  }
  for (from, to) in moved.iter().cloned() {
    hub.broadcast("file.changed".into(), serde_json::json!({"path": to, "kind":"moved", "fromPath": from}));
    hub.inc_file_changed();
  }
  ...
  hub.broadcast("tree.changed".into(), serde_json::json!({"impactedPaths": impacted_list.clone(), "summary": summary }));
  ...
  if std::env::var("AILOOM_FSWATCH_FORCE_RESYNC").unwrap_or_else(|_| "0".into()) == "1" {
    hub.broadcast_ephemeral("session.resync".into(), serde_json::json!({"reason":"fswatch_force"}));
    tracing::info!(target:"fswatch", "force session.resync broadcast");
  }
}

// --- ignore helpers ---
struct IgnoreFilter { ig: ignore::gitignore::Gitignore }
...
fn build_ignore_filter(root: &PathBuf, cfg: &Cfg) -> Option<IgnoreFilter> {
  ...
  if cfg.ignore_vcs {
    ...
    if gi.exists() { b.add(gi); has_any = true; }
  }
  if cfg.ignore_ailoom {
    let ai = root.join(".ailoomignore");
    if ai.exists() { b.add(ai); has_any = true; }
  }
  if !has_any { return None; }
  match b.build() {
    Ok(ig) => Some(IgnoreFilter { ig }),
    Err(e) => { tracing::warn!(target: "fswatch", "ignore build failed: {}", e); None }
  }
}
...
fn is_ignored(filter: Option<&IgnoreFilter>, rel_path: &str) -> bool {
  ...
  // hard excludes
  rel_path.starts_with(".git/") || rel_path == ".git" || rel_path.starts_with("node_modules/") || rel_path == "node_modules"
}
