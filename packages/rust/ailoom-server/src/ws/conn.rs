...
use crate::{state::AppState, ws::{hub::{Event, Hub}, protocol::{RpcNotification, RpcRequest, RpcResponse}, config::WsConfig}};

#[derive(Clone)]
struct SubFilter { topic: String, filter: Value }
...
pub async fn handle_connection(state: AppState, mut socket: WebSocket) {
  ...
  let welcome = RpcNotification { jsonrpc: "2.0".into(), method: "session.welcome".into(), params: Some(json!({
  ...
  if trace { tracing::info!(target:"ws", conn=%conn_id, "welcome sent"); }

  // subscriptions: token -> SubFilter
  let subs: Arc<tokio::sync::RwLock<HashMap<String, SubFilter>>> = Arc::new(tokio::sync::RwLock::new(HashMap::new()));
  ...
  let forward_task = tokio::spawn(async move {
    let Some(hub_inst) = hub_for_fwd.clone() else { return };
    let mut rx = hub_inst.subscribe();
    let mut last_sent_event_id: u64 = 0;
    loop {
      if closed_for_fwd.load(Ordering::Relaxed) { break; }
      match rx.recv().await {
        Ok(ev) => {
          let mut allowed = {
          ...
          if ev.method == "session.stats" || ev.method == "file.changed" || ev.method == "tree.changed" || ev.method == "session.resync" || unfiltered { allowed = true; }
          if !allowed { continue; }
          if ev.method == "file.changed" || ev.method == "tree.changed" || ev.method.starts_with("annotations.") || ev.method == "session.resync" {
            let now = time::OffsetDateTime::now_utc().unix_timestamp_nanos() as u64 / 1_000_000;
            last_biz_enqueued_ms_for_fwd.store(now, Ordering::Relaxed);
          }
          // 注意：不要在转发协程中更新 last_sent_event_id_shared（仅在“实际写出成功”后更新）
          if let Some(eid) = ev.params.get("eventId").and_then(|v| v.as_str()).and_then(|s| s.parse::<u64>().ok()) {
            if eid > last_sent_event_id { last_sent_event_id = eid; }
          } else if let Some(eid) = ev.params.get("eventId").and_then(|v| v.as_u64()) {
            if eid > last_sent_event_id { last_sent_event_id = eid; }
          }
          let notif = RpcNotification { jsonrpc: "2.0".into(), method: ev.method.clone(), params: Some(ev.params) };
          let text = serde_json::to_string(&notif).unwrap();
          if trace { let eid_dbg = last_sent_event_id; tracing::info!(target:"ws", conn=%conn_id, event_id=%eid_dbg, method=%notif.method, "fwd"); }
          if notif.method == "tree.changed" {
            let _ = events_tree_tx_hub.send_replace(Some(text));
          } else if notif.method == "file.changed" || notif.method.starts_with("annotations.") || notif.method == "session.resync" {
        ...
        Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {
          if let Some(hub) = hub_for_fwd.clone() {
            ...
            if events.is_empty() && after == 0 {
              let notif = RpcNotification { jsonrpc: "2.0".into(), method: "session.resync".into(), params: Some(json!({"reason":"lagged_initial"})) };
              let _ = priority_tx_fwd.send(serde_json::to_string(&notif).unwrap()).await;
              ...
              for e in events.into_iter() {
                let mut allowed = {
                  let guard = subs_for_fwd.read().await;
                  let ev_tmp = crate::ws::hub::Event { method: e.method.clone(), params: e.params.clone() };
                  subs_match_any(&*guard, &ev_tmp)
                };
                if e.method == "session.stats" || e.method == "file.changed" || e.method == "tree.changed" || e.method == "session.resync" || unfiltered { allowed = true; }
                if !allowed { continue; }
                let notif = RpcNotification { jsonrpc: "2.0".into(), method: e.method.clone(), params: Some(e.params.clone()) };
                let text = serde_json::to_string(&notif).unwrap();
                if notif.method == "tree.changed" {
                  let _ = events_tree_tx_hub.send_replace(Some(text));
                } else if notif.method == "file.changed" || notif.method.starts_with("annotations.") || notif.method == "session.resync" {
                ...
                if e.id > last_sent_event_id { last_sent_event_id = e.id; }
              }
            }
          }
          continue;
    ...
    if let Some(hub) = hub_for_pump.clone() {
      loop {
        ...
        for e in events.into_iter() {
          ...
          if trace_pump { tracing::info!(target:"ws", event_id=%e.id, method=%e.method, "pump"); }
        }
      }
    }
  });
  ...
  if force_recover {
    ...
    tokio::spawn(async move {
      loop {
        ...
        if closed_for_recover.load(Ordering::Relaxed) { break; }
        let enq_ms = last_biz_enqueued_ms_chk.load(Ordering::Relaxed);
        if enq_ms == 0 { continue; }
        let ok_ms = last_biz_write_ok_ms_chk.load(Ordering::Relaxed);
        if ok_ms >= enq_ms { continue; }
        let now = time::OffsetDateTime::now_utc().unix_timestamp_nanos() as u64 / 1_000_000;
        if now.saturating_sub(enq_ms) > force_recover_ms {
          if !close_first {
            // 兼容旧策略：先尝试发 resync（通过优先 chan）
            let notif = RpcNotification { jsonrpc: "2.0".into(), method: "session.resync".into(), params: Some(json!({"reason":"force_recover"})) };
            let _ = priority_tx_recover.send(serde_json::to_string(&notif).unwrap()).await;
          }
          closed_for_recover.store(true, Ordering::Relaxed);
          // 立即唤醒，尽快退出读循环；发送协程会因通道关闭或后续错误而退出
          shutdown_recover.notify_waiters();
          if trace { tracing::info!(target:"ws", conn=%conn_id, close_first=%close_first, "force_recover: close connection"); }
          break;
        }
      }
    });
  ...
  if enable_supervisor {
    tokio::spawn(async move {
      if let Some(hub) = hub_for_super.clone() {
        ...
        let close_first = std::env::var("AILOOM_WS_RECOVER_CLOSE_FIRST").unwrap_or_else(|_| "1".into()) == "1";
        loop {
        tokio::time::sleep(Duration::from_millis(cfg.pump_ms.min(300))).await;
          if closed_for_super.load(Ordering::Relaxed) { break; }
          let hub_last = hub.stats_snapshot().last_event_id;
          let conn_last = last_sent_event_id_super.load(Ordering::Relaxed);
          if hub_last <= conn_last { continue; }
          // 本连接落后于 Hub；给它一个窗口（force_recover_ms_super）让其写出；若超时仍未追上，则下发 resync
          let deadline = time::OffsetDateTime::now_utc().unix_timestamp_nanos() as u64 / 1_000_000 + force_recover_ms_super;
          // 简单等待窗口
          tokio::time::sleep(Duration::from_millis(force_recover_ms_super.min(400))).await;
          if closed_for_super.load(Ordering::Relaxed) { break; }
          ...
          if hub_last > conn_after && now_ms >= deadline {
            if !close_first {
              let notif = RpcNotification { jsonrpc: "2.0".into(), method: "session.resync".into(), params: Some(json!({"reason":"supervisor"})) };
              let _ = priority_tx_super.send(serde_json::to_string(&notif).unwrap()).await;
            }
            // 标记关闭并唤醒各循环，加速断开重连
            closed_for_super.store(true, Ordering::Relaxed);
            shutdown_super.notify_waiters();
            // supervisor 场景下打印关键游标对比
            if trace { tracing::info!(target:"ws", conn=%conn_id, hub_last=%hub_last, conn_last=%conn_after, close_first=%close_first, "supervisor: close connection"); }
            break;
          }
        }
      }
  ...
  let ping_task = tokio::spawn(async move {
    loop {
      ...
      let notif = RpcNotification { jsonrpc: "2.0".into(), method: "session.ping".into(), params: Some(json!({"ts": time::OffsetDateTime::now_utc().format(&time::format_description::well_known::Rfc3339).unwrap_or_else(|_| "".into())})) };
      let _ = out_tx_ping.send(serde_json::to_string(&notif).unwrap()).await;
    }
  });
  ...
  for (_t, s) in subs.iter() {
    if s.topic == "file" && ev.method == "file.changed" {
      if let Some(p) = ev.params.get("path").and_then(|v| v.as_str()) {
        let f = &s.filter;
        if let Some(pp) = f.get("path").and_then(|v| v.as_str()) { if pp == p { return true; } }
        if let Some(pref) = f.get("prefix").and_then(|v| v.as_str()) { if p.starts_with(pref) { return true; } }
      }
      ...
      if let Some(dir) = ev.params.get("dir").and_then(|v| v.as_str()) { if dir == want { return true; } }
      if let Some(imps) = ev.params.get("impactedPaths").and_then(|v| v.as_array()) {
        for it in imps { if let Some(pp) = it.as_str() { if pp.starts_with(&(want.to_string() + "/")) || pp == want { return true; } } }
      }
    } else if s.topic == "annotations" && ev.method.starts_with("annotations.") {
      let want = s.filter.get("filePath").and_then(|v| v.as_str());
      if want.is_none() { return true; }
      let wantp = want.unwrap();
      if ev.method == "annotations.deleted" { return true; }
      if let Some(obj) = ev.params.get("annotation").and_then(|v| v.as_object()) {
        if let Some(fp) = obj.get("filePath").and_then(|v| v.as_str()) { if fp == wantp { return true; } }
      }
    }
  }
  false
...
async fn handle_request(state: &AppState, subs: &Arc<tokio::sync::RwLock<HashMap<String, SubFilter>>>, req: RpcRequest) -> RpcResponse {
  ...
  match req.method.as_str() {
    "subscribe" => {
      ...
      if topic.is_empty() { return RpcResponse::err(id, "INVALID_PARAMS", "topic required", None); }
      let token = format!("{}:{}", topic, normalize_filter(&filter));
      {
        let mut guard = subs.write().await;
        guard.entry(token.clone()).or_insert(SubFilter { topic: topic.to_string(), filter });
      }
      RpcResponse::ok(id, json!({"token": token}))
    }
    "unsubscribe" => {
      let tok = params.get("token").and_then(|v| v.as_str());
      if let Some(t) = tok { let mut guard = subs.write().await; guard.remove(t); }
